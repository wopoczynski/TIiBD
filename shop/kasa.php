...
END;
    ...
    <div class = "kafelek">
        ...
        <?php
            ...
            if (isset($_POST['kasa'])){
				$numer =$_POST['numer'];
				$ile_prod=$_POST['ile_prod'];
				if($ile_prod==0){
					echo "<center>Koszyk jest pusty!</center>";
				}
				else{
				$query="UPDATE users_buy_products SET state='1' WHERE id_u = $numer";
				mysqli_query($link,$query);
				echo "<center>Zlecenie przekazano do realizacji</center>";
				
				}
			 }
			  if (isset($_POST['pusty'])){
				$numer =$_POST['numer'];
				
				$info=mysqli_query($link,"SELECT * FROM users_buy_products WHERE id_u = $numer");
				$rowrow = mysqli_fetch_assoc($info);
				$niekup = $rowrow['quantity'];
				$produkt=$rowrow['id_p'];
				...
				echo "<center>Koszyk został opróżniony</center>";
			 }
